...
public class DBManager{
  public static  string connString=@"server=localhost;port=3306;user=root;password=root; database=practice";
    ...
    public static bool UpdateData(int id, String nameFirst, String namelast, String email){
        ...
        MySqlConnection conn= new MySqlConnection();
        conn.ConnectionString=connString;
        String query="insert into students values(@id1,@first,@last,@email)";
        MySqlCommand cmd=new MySqlCommand(query,conn);
        conn.Open();
        cmd.Parameters.AddWithValue("@first",nameFirst);
        cmd.Parameters.AddWithValue("@last",namelast);
     ...
     public static bool DeleteData(int id){
        MySqlConnection conn= new MySqlConnection();
        conn.ConnectionString=connString;
        String query="delete from Students where id=@id1";
        MySqlCommand cmd=new MySqlCommand(query,conn);
        conn.Open();
